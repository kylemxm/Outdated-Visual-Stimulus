function angle_list = Jenks_Reversing_Grating(start_angle, f, drawmask, gratingsize,repetition,duration)
%hacked together program to do Reversing grating. 
%Jenks_Reversing_Grating(start_angle, cyclespersecond, f, drawmask, gratingsize,repetition,duration)
...
%
% Made by Kyle Jenks for the use of the Shepherd lab at the University of
% Utah, 10-21-16
...
if isempty(start_angle)
    ...
    start_angle=[0,30,60,90,120,150];
end;
if nargin < 1
    repetition = [];
end
...
% First we compute pixels per cycle, rounded up to full pixels, as we
    ...
    grating1=gray + inc*cos(fr*x);
    grating2=(gray + inc*cos(fr*x))*-1;

    % Store 1-D single row grating in texture:
    gratingtex(1)=Screen('MakeTexture', w, grating1);
    gratingtex(2)=Screen('MakeTexture', w, grating2);
...
%% Start presentations 
    ...
    % Time we want to wait before reversing the contrast of the grating
checkFlipTimeSecs = .5; %time to wait before flip
...
frameCounter = 0;
    ...
    frameCounter = frameCounter + 1;
        ...
        Screen('DrawTexture', w, gratingtex(textureCue(1)),[], dstRect, angle(ii));
        if drawmask==1
